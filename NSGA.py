...
class NSGA(object):
    def __init__(self):
        ...
        for j in range(self.population_size):
            ...
            for i in range(globalv.T_num):
                gene[3, i * 3] = globalv.S[random.randint(0, len(globalv.S) - 1)]
                gene[3, i * 3 + 1] = globalv.A[random.randint(0, len(globalv.A) - 1)]
                gene[3, i * 3 + 2] = globalv.S[random.randint(0, len(globalv.S) - 1)]
                # 第五行：所需资源
                gene[4, (i - 1) * 3] = 0
                gene[4, (i - 1) * 3 + 2] = 0
                gene[4, (i - 1) * 3 + 1] = globalv.Tar_need[i - 1]
            # 转换回去
            # 第一行：自然数
